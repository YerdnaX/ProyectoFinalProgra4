...
/* ----------------------------------------------------------
   ...
   Crea un cliente en Banco C (usado para replicacion desde Banco A).
   Es idempotente: si el cliente ya existe retorna ok sin error.
   ---------------------------------------------------------- */
router.post('/clientes/crear', async function (req, res) {
  try {
    ...
    if (Number(existeResult.recordset[0].total) > 0) {
      return res.json({ ok: true, action: 'exists', message: 'El cliente ya existe en Banco C.' });
    }
    ...
    return res.json({ ok: true, action: 'created', message: 'Cliente creado en Banco C.' });
  } catch (error) {
    return res.status(500).json({
      ok: false,
      reason: 'INTERNAL_ERROR',
      message: 'Error creando cliente en Banco C.'
    });
  }
});
...
router.post('/cuentas/crear', async function (req, res) {
  try {
    ...
    try {
      ...
      console.warn('Banco C: No se pudo registrar en Banco Central:', errCentral.message);
    }
